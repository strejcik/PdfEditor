// server.js
// Static frontend + Socket.IO live server with JWT-secured room policy
...
const rooms = new Map(); // roomId -> { createdAt, hostSid, maxViewers, expiresAt, viewers:Set<string> }
const nowSec = () => Math.floor(Date.now() / 1000);

// ---------------------------- jwt helpers ----------------------------
function signToken(payload, ttlSec) {
  return jwt.sign(payload, LIVE_JWT_SECRET, { algorithm: "HS256", expiresIn: ttlSec });
}
...
// POST /live/create { room?, maxViewers?, ttlMinutes? }
app.post("/live/create", (req, res) => {
  const proposed = (req.body && req.body.room) || null;
  const room = proposed && /^[A-Za-z0-9_-]{4,32}$/.test(proposed)
    ? proposed
  ...
  rooms.set(room, { createdAt: nowSec(), hostSid: null, maxViewers, expiresAt, viewers: new Set() });
...
app.post("/live/invite", (req, res) => {
  ...
  let dec;
  try { dec = verifyToken(m[1]); } catch { return res.status(401).json({ error: "invalid token" }); }
  if (dec.kind !== "live" || dec.role !== "host" || !dec.room) return res.status(403).json({ error: "forbidden" });
  if (!rooms.has(dec.room)) return res.status(404).json({ error: "room not found" });

  // 10-minute viewer invite
  const viewerToken = signToken({ kind: "live", role: "viewer", room: dec.room }, 10 * 60);
...
app.get("/live/rooms/:room", (req, res) => {
  ...
  res.json({ ok: true, hasHost: !!r.hostSid, viewers: r.viewers.size, maxViewers: r.maxViewers, expiresAt: r.expiresAt });
});
...
io.on("connection", (socket) => {
  ...
  socket.on("join", (payload = {}, ack) => {
    ...
    if (!room || !token) return ack?.({ ok: false, error: "missing room or token" });

    // Validate token
    let dec;
    try { dec = verifyToken(token); }
    catch (e) { return ack?.({ ok: false, error: "invalid token" }); }
    ...
    if (asHost) {
      if (dec.role !== "host") return ack?.({ ok: false, error: "need host token" });
      ...
      if (dec.role !== "viewer") return ack?.({ ok: false, error: "need viewer token" });
  ...
  socket.on("host_state", (msg) => {
    ...
    // Expect { schemaVersion, state } or whatever your client sends
  ...
  socket.on("request_full", (msg) => {
    const room = socket.data.room;
    if (!room) return;
    // In a simple setup, just broadcast to host in the same room:
...
server.listen(PORT, () => {
  ...
  console.log(`JWT secret: ${LIVE_JWT_SECRET ? "SET" : "DEFAULT (INSECURE)"}`);
});
